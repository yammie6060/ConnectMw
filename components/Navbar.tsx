...
export default function Navbar() {
  ...
  return (
    <>
        ...
        <ul className="hidden md:flex items-center gap-8 list-none">
          {navLinks.map((link) => (
            <li key={link.href}>
              <a
                href={link.href}
                className="text-[#cde0f0] no-underline text-sm font-medium transition-colors duration-200 hover:text-[#f5ab20]"
              >
                {link.label}
              </a>
            </li>
      ...
      <div
        className={`fixed top-[68px] left-0 right-0 z-[999] bg-[#132333] border-b border-[rgba(245,166,35,0.12)] transition-all duration-300 md:hidden ${
        ...
        <div className="px-[6%] py-6 flex flex-col gap-4">
          {navLinks.map((link) => (
            <a
              ...
              className="text-[#cde0f0] no-underline text-base font-medium py-2 border-b border-[rgba(255,255,255,0.06)] hover:text-[#f5ab20] transition-colors"
            >
              {link.label}
            </a>
          ))}
          <div className="flex gap-3 mt-2">
            <button
              ...
              className="flex-1 px-4 py-2.5 rounded-full text-sm font-medium text-white border border-[rgba(255,255,255,0.25)] bg-transparent hover:border-[#f5ab20] hover:text-[#f5ab20] transition-all cursor-pointer"
            >
              Sign In
            </button>
            <button
              onClick={() => { router.push("/signup"); setMobileOpen(false); }}
              className="flex-1 px-4 py-2.5 rounded-full text-sm font-bold text-[#0d1f2d] bg-[#f5ab20] border-none hover:bg-[#e8941a] transition-all cursor-pointer"
            >
              Sign Up Free
            </button>
          </div>
